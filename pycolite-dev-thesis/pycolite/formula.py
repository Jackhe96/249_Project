...
def transform_eventually(curr_node, n, negated=False, orthogonalize=False):
    curr_has_left = has_left_child(curr_node)
    curr_has_right = has_right_child(curr_node)
    if not curr_has_left and not curr_has_right:
        return curr_node

    if curr_node.Symbol == "NOT":
        negated = not negated

    if curr_has_left:
        curr_node.left_formula = transform_eventually(curr_node.left_formula, n, negated, orthogonalize)
    if curr_has_right:
        curr_node.right_formula = transform_eventually(curr_node.right_formula, n, negated, orthogonalize)
    
    if curr_node.Symbol == "EVENTUALLY" and not negated:
        if orthogonalize == True:
            print "orthogonalizing"
            return __orthogonalize_timing(curr_node.right_formula, n)
        print "not orthogonalizing"
        return __do_transformation(curr_node.right_formula, n)
    return curr_node

def __do_transformation(curr_node, n):
    c = tmp = curr_node
    for _ in range(n):
        tmp = Next(tmp)
        c = Disjunction(c, tmp)
...
def __orthogonalize_timing(curr_node, n):
    c = curr_node
    for _ in range(n):
        c = Next(c)
    return c

def has_left_child(curr_node):
    return hasattr(curr_node, "left_formula") and curr_node.left_formula != None

def has_right_child(curr_node):
    return hasattr(curr_node, "right_formula") and curr_node.right_formula != None